...
function default_settings() {
  ...
  echo -e "${RAMSIZE}${BOLD}${DGN}RAM Size: ${BGN}8GB (${RAM_SIZE} MiB)${CL}"
...
function advanced_settings() {
  ...
  if RAM_GB=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Allocate RAM in GB (e.g., 2, 4, 8, 16)" 8 58 2 --title "RAM (GB)" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $RAM_GB ]; then
      RAM_GB="2"
    fi
